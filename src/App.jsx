...
function CapacityCards({eu,HPM,fmtH,month,fmtLong}){
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8,marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <select value={capDept} onChange={e=>{setCapDept(e.target.value);setOpenId(null);}} style={selStyle}>
            ...
            <option value="Creative Department">Creative</option>
            <option value="Client Servicing Department">Client Servicing</option>
            <option value="Production Department">Production</option>
            <option value="Planning Department">Planning</option>
          </select>
...
function DashboardPage(){
  ...
  const C=useMemo(()=>{
    ...
    const eu=dbEmployees.map(e=>{const empAls=als.filter(a=>(a.eid||a.employee_id)===e.id);const h=empAls.reduce((s,a)=>s+(parseFloat(a.h||a.allocated_hours)||0),0);const clients=empAls.filter(a=>a.status!=='On Leave'&&parseFloat(a.h||a.allocated_hours)>0).map(a=>({name:a.client_name||a.cn||'',hours:parseFloat(a.h||a.allocated_hours)||0}));return{...e,h,u:(h/HPM)*100,av:Math.max(0,HPM-h),clients};});
    ...
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      {finTab==="team"&&(
        <div style={{display:"flex",flexDirection:"column",gap:18}}>
          ...
          <CapacityCards eu={C.eu} HPM={HPM} fmtH={fmtH} month={month} fmtLong={fmtLong}/>
        </div>
      )}
    </div>
  );
...
function AllocationsPage(){
  ...
  const filtered=useMemo(()=>allocs.filter(a=>{
    ...
    return ms&&(filterEmp==="all"||a.employee_id===filterEmp)&&(filterCli==="all"||a.client_id===filterCli)&&(filterDept==="all"||emp?.department===filterDept)&&(filterCat==="all"||isOnLeave||(ct?.contract_category||"Retainer")===filterCat);
