    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,#0f172a 0%,#1e293b 50%,#0f172a 100%)"}}>
      <div style={{width:"100%",maxWidth:400,padding:"0 20px"}}>
        ...
        <div style={{textAlign:"center",marginBottom:36}}>
          ...
          <p style={{margin:"6px 0 0",fontSize:13,color:"#777777"}}>Acquaint Communications</p>
        </div>

        {/* Card */}
        <div style={{background:"#161616",borderRadius:20,padding:32,boxShadow:"0 25px 50px rgba(0,0,0,.4)"}}>
          <h2 style={{margin:"0 0 6px",fontSize:18,fontWeight:700,color:"#FFFFFF"}}>Sign in to your account</h2>
          <p style={{margin:"0 0 24px",fontSize:13,color:"#777777"}}>Enter your credentials to continue</p>
          ...
          <form onSubmit={handleLogin} style={{display:"flex",flexDirection:"column",gap:14}}>
            <div>
              <label style={{display:"block",fontSize:13,fontWeight:600,color:"#B3B3B3",marginBottom:6}}>Email address</label>
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)} required placeholder="you@company.com"
                style={{width:"100%",padding:"11px 14px",border:"1px solid #262626",borderRadius:10,fontSize:14,color:"#FFFFFF",outline:"none",boxSizing:"border-box",transition:"border-color .2s"}}
                onFocus={e=>e.target.style.borderColor="#1DC99A"} onBlur={e=>e.target.style.borderColor="#e5e7eb"}/>
            </div>
            <div>
              <label style={{display:"block",fontSize:13,fontWeight:600,color:"#B3B3B3",marginBottom:6}}>Password</label>
              <input type="password" value={password} onChange={e=>setPassword(e.target.value)} required placeholder="••••••••"
                style={{width:"100%",padding:"11px 14px",border:"1px solid #262626",borderRadius:10,fontSize:14,color:"#FFFFFF",outline:"none",boxSizing:"border-box",transition:"border-color .2s"}}
                onFocus={e=>e.target.style.borderColor="#1DC99A"} onBlur={e=>e.target.style.borderColor="#e5e7eb"}/>
            </div>
            <button type="submit" disabled={loading}
              style={{padding:"12px",borderRadius:10,border:"none",background:"#1DC99A",color:"#000",fontSize:15,fontWeight:700,cursor:loading?"not-allowed":"pointer",opacity:loading?.7:1,marginTop:4,transition:"opacity .2s"}}>
              {loading?"Signing in…":"Sign In →"}
            </button>
          </form>

          <p style={{margin:"20px 0 0",fontSize:12,color:"#777777",textAlign:"center"}}>
...
function DashboardPage(){
  ...
  const KPI=({label,value,sub,emoji,dark,iBg,iC,bg,bd})=>(
    <Card style={{background:dark?"#0A0A0A":bg||"#161616",borderColor:bd||"#e2e8f0",padding:20}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div>
          <p style={{fontSize:10,color:dark?"#777777":"#777777",margin:0,textTransform:"uppercase",letterSpacing:".05em"}}>{label}</p>
          <p style={{fontSize:18,fontWeight:800,color:dark?"#FFFFFF":iC||"#FFFFFF",margin:"5px 0 3px"}}>{value}</p>
          <p style={{fontSize:10,color:dark?"#777777":"#777777",margin:0}}>{sub}</p>
        </div>
        <div style={{background:dark?"#1E1E1E":iBg||"#262626",borderRadius:10,width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,flexShrink:0}}>{emoji}</div>
      </div>
    </Card>
  );
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10}}>
        <div><h1 style={{fontSize:26,fontWeight:800,color:"#FFFFFF",margin:0}}>Dashboard</h1><p style={{fontSize:13,color:"#777777",marginTop:3}}>Overview of finance and team metrics</p></div>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <span style={{fontSize:13,color:"#777777"}}>📅</span>
          <Sel value={month} onChange={setMonth} options={MONTHS.map(m=>({v:m,l:fmtLong(m)}))} style={{width:160}}/>
        </div>
      </div>

      {/* Tabs */}
      <div style={{display:"flex",gap:4,background:"#1E1E1E",borderRadius:10,padding:4,maxWidth:380}}>
      ...
      {finTab==="finance"&&(
        <div style={{display:"flex",flexDirection:"column",gap:18}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10}}>
            ...
            <KPI label="Total Contract Value" value={SAR(C.tcv)} sub="All active contracts"  emoji="📋" iBg="#f3e8ff" iC="#1DC99A"/>
            <KPI label="Gross Profit"        value={SAR(C.tp)}  sub={`${C.am.toFixed(1)}% margin`} emoji="📊" iBg={C.tp>=0?"#d1fae5":"#fee2e2"} iC={C.tp>=0?"#22C55E":"#EF4444"} bg={C.tp>=0?"#f0fdf4":"#fff5f5"} bd={C.tp>=0?"#a7f3d0":"#fecaca"}/>
            <KPI label="Active Contracts"    value={C.acnt}     sub={`${C.lm} low margin`}  emoji="📄" iBg="#0d1f1a" iC="#1DC99A"/>
          </div>
          <Card style={{padding:20}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8,marginBottom:12}}>
              <p style={{margin:0,fontWeight:700,fontSize:13,color:"#FFFFFF"}}>LifeTime — Monthly Retainer vs Resource Cost (Closed Months)</p>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={ltd} margin={{top:5,right:15,left:5,bottom:5}}>
                ...
                <Legend wrapperStyle={{fontSize:11,color:"#777777"}}/>
                <Bar dataKey="retainer" name="Contract Value" fill="#1DC99A" radius={[4,4,0,0]}/>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
            <Card style={{padding:18}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,marginBottom:10}}>
                <p style={{margin:0,fontWeight:700,fontSize:12,color:"#FFFFFF"}}>Monthly Retainer vs Monthly Cost</p>
              ...
              <ResponsiveContainer width="100%" height={190}>
                <BarChart data={rd} margin={{top:5,right:5,left:0,bottom:36}}>
                  ...
                  <Tooltip formatter={v=>SAR(v)} contentStyle={{borderRadius:8,border:"1px solid #262626",background:"#161616",color:"#FFFFFF"}}/>
                  <Legend wrapperStyle={{fontSize:10,color:"#777777"}}/>
                  <Bar dataKey="monthly" name="Monthly Retainer" fill="#1DC99A" radius={[4,4,0,0]}/>
            ...
            <Card style={{padding:18}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,marginBottom:10}}>
                <p style={{margin:0,fontWeight:700,fontSize:12,color:"#FFFFFF"}}>Department Budget vs Cost</p>
              ...
              <ResponsiveContainer width="100%" height={190}>
                <BarChart data={C.dbc(dc)} margin={{top:5,right:5,left:0,bottom:8}}>
                  ...
                  <Tooltip formatter={v=>SAR(v)} contentStyle={{borderRadius:8,border:"1px solid #262626",background:"#161616",color:"#FFFFFF"}}/>
                  <Legend wrapperStyle={{fontSize:10,color:"#777777"}}/>
                  <Bar dataKey="budget" name="Budget"      fill="#1DC99A" radius={[4,4,0,0]}/>
          ...
          <Card style={{overflow:"hidden"}}>
            <div style={{padding:"13px 18px",borderBottom:"1px solid #1E1E1E"}}><p style={{margin:0,fontWeight:700,fontSize:13,color:"#FFFFFF"}}>Client Profitability</p></div>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr style={{background:"#161616"}}>{["Client","Retainer","Cost","Profit","Margin"].map((h,i)=><th key={h} style={{padding:"7px 14px",textAlign:i===0?"left":i===4?"center":"right",fontSize:11,fontWeight:600,color:"#777777",borderBottom:"1px solid #262626"}}>{h}</th>)}</tr></thead>
              <tbody>{C.cp.slice(0,5).map(c=>{const neg=c.gp<0,low=c.mp<20;return(<tr key={c.id} style={{borderBottom:"1px solid #f8fafc",background:neg?"#1a0808":low?"#1a1000":"#161616"}}>
                <td style={{padding:"9px 14px",fontWeight:600,fontSize:13,color:"#FFFFFF"}}>{c.name}</td>
                <td style={{padding:"9px 14px",textAlign:"right",fontSize:13}}>{SAR(c.mr)}</td>
                <td style={{padding:"9px 14px",textAlign:"right",fontSize:13,color:"#777777"}}>{SAR(c.rc)}</td>
                <td style={{padding:"9px 14px",textAlign:"right",fontSize:13,fontWeight:700,color:neg?"#EF4444":"#22C55E"}}>{SAR(c.gp)}</td>
                <td style={{padding:"9px 14px",textAlign:"center"}}><Bdg bg={neg?"#fee2e2":low?"#fef9c3":"#d1fae5"} color={neg?"#EF4444":low?"#d97706":"#22C55E"}>{c.mp.toFixed(0)}%</Bdg></td>
              </tr>);})}</tbody>
            </table>
          </Card>
        </div>
      ...
      {finTab==="team"&&(
        <div style={{display:"flex",flexDirection:"column",gap:18}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
            ...
            <KPI label="Renewals"        value={C.ren.length}   sub="Within 60 days" emoji="📅" iBg="#0d1f1a" iC="#1DC99A" bg={C.ren.length>0?"#0d1f1a":"#161616"} bd={C.ren.length>0?"#1DC99A44":"#262626"}/>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:14}}>
            <Card style={{padding:18}}>
              <p style={{margin:"0 0 10px",fontWeight:700,fontSize:13,color:"#FFFFFF"}}>Team Utilization</p>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={C.chart.slice(0,8)} layout="vertical" margin={{top:5,right:20,left:42,bottom:5}}>
                  ...
                  <Tooltip formatter={(v,n)=>[`${v} hrs`,n==="hours"?"Allocated":"Available"]} contentStyle={{borderRadius:8,border:"1px solid #262626",background:"#161616",color:"#FFFFFF"}}/>
                  <Bar dataKey="hours" stackId="a" name="Allocated">{C.chart.slice(0,8).map((e,i)=><Cell key={i} fill={e.u>100?"#ef4444":e.u>=70?"#1DC99A":"#f59e0b"}/>)}</Bar>
                  <Bar dataKey="available" stackId="a" fill="#e2e8f0" name="Available"/>
                </BarChart>
              </ResponsiveContainer>
            </Card>
            <Card style={{padding:18}}>
              <p style={{margin:"0 0 10px",fontWeight:700,fontSize:13,color:"#FFFFFF"}}>Contract Renewals</p>
              {C.ren.length===0?(<div style={{textAlign:"center",padding:"20px 0",color:"#777777"}}><div style={{fontSize:28,marginBottom:6}}>📅</div><p style={{fontSize:12}}>No renewals in 60 days</p></div>):(
                <div style={{display:"flex",flexDirection:"column",gap:7}}>
                  {C.ren.slice(0,4).map(c=>{const d=diffDays(c.ed),urg=d<=7,warn=d<=30&&!urg;return(<div key={c.id} style={{padding:10,borderRadius:9,border:`1px solid ${urg?"#EF444444":warn?"#F59E0B44":"#262626"}`,background:"#161616",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div><p style={{margin:0,fontWeight:600,fontSize:12,color:"#FFFFFF"}}>{c.cn}</p><p style={{margin:"1px 0 0",fontSize:10,color:"#777777"}}>{fmtDate(c.ed)}</p></div>
          ...
          <Card style={{padding:18}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:8,marginBottom:14}}>
              <div><p style={{margin:0,fontWeight:700,fontSize:13,color:"#FFFFFF"}}>Team Capacity</p><p style={{margin:"1px 0 0",fontSize:10,color:"#777777"}}>{fmtLong(month)} · {HPM}h/person</p></div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(110px,1fr))",gap:8}}>
              {cf.map(emp=>{const ov=emp.u>100,un=emp.u<70;return(
                <div key={emp.id} style={{padding:10,borderRadius:11,border:`1px solid ${ov?"#EF444444":un?"#F59E0B44":"#22C55E44"}`,background:"#161616"}}>
                  <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:5}}>
                    <div style={{width:22,height:22,borderRadius:6,background:"#1E1E1E",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,fontWeight:700,color:ov?"#EF4444":un?"#d97706":"#22C55E",flexShrink:0}}>{emp.name.slice(0,2).toUpperCase()}</div>
                    <p style={{margin:0,fontSize:10,fontWeight:600,color:"#FFFFFF",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flex:1}}>{emp.name.split(" ")[0]}</p>
                    {ov&&<span style={{fontSize:9}}>⚠️</span>}
                  </div>
                  <PBar val={emp.u} color={ov?"#ef4444":un?"#f59e0b":"#1DC99A"}/>
                  <div style={{display:"flex",justifyContent:"space-between"}}>
                    <span style={{fontSize:10,fontWeight:700,color:ov?"#EF4444":un?"#d97706":"#22C55E"}}>{Math.min(emp.u,999).toFixed(0)}%</span>
                    <span style={{fontSize:9,color:"#777777"}}>{emp.h}h</span>
                  </div>
                </div>
              );})}
            </div>
...
function EmployeesPage(){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10}}>
        <div><h1 style={{fontSize:26,fontWeight:800,color:"#FFFFFF",margin:0}}>Employees</h1><p style={{fontSize:13,color:"#777777",marginTop:3}}>Manage your team members and their costs</p></div>
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
        <div style={{position:"relative",flex:1,minWidth:180}}>
          <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#777777",fontSize:13}}>🔍</span>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search employees..." style={{width:"100%",padding:"8px 12px 8px 30px",border:"1px solid #262626",borderRadius:9,fontSize:13,outline:"none",background:"#161616",boxSizing:"border-box"}}/>
        </div>
        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead><tr>
              ...
              <th style={{padding:"9px 13px",textAlign:"right",fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626"}}>Hourly Rate</th>
              <SortTh k="status" sk={sk} sd={sd} onSort={sort} align="center">Status</SortTh>
              <th style={{padding:"9px 13px",textAlign:"right",fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626"}}>Actions</th>
            </tr></thead>
            <tbody>
              {sorted.map((emp,idx)=>{
                const hr=emp.mc/HPM;
                const sb=emp.status==="Active"?"#d1fae5":emp.status==="Inactive"?"#f1f5f9":"#fef9c3";
                const sc2=emp.status==="Active"?"#22C55E":emp.status==="Inactive"?"#64748b":"#d97706";
                return(<tr key={emp.id} style={{borderBottom:"1px solid #1E1E1E",background:idx%2===0?"#161616":"#111111"}}>
                  <td style={{padding:"11px 13px"}}><div style={{display:"flex",alignItems:"center",gap:10}}><Avatar name={emp.name}/><div><p style={{margin:0,fontWeight:600,fontSize:13,color:"#FFFFFF"}}>{emp.name}</p><p style={{margin:"1px 0 0",fontSize:11,color:"#777777"}}>{emp.designation}</p></div></div></td>
                  <td style={{padding:"11px 13px"}}><Bdg>{emp.department?.replace(" Department","")}</Bdg></td>
                  <td style={{padding:"11px 13px",fontSize:13,color:"#B3B3B3"}}>📍 {emp.location}</td>
                  <td style={{padding:"11px 13px",textAlign:"right",fontWeight:600,fontSize:13,color:"#FFFFFF"}}>{SAR(emp.mc)}</td>
                  <td style={{padding:"11px 13px",textAlign:"right",fontSize:13,color:"#777777"}}>SAR {hr.toFixed(0)}/hr</td>
                  <td style={{padding:"11px 13px",textAlign:"center"}}>
                    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
                      <Bdg bg={sb} color={sc2}>{emp.status}</Bdg>
                      {emp.status==="Inactive"&&emp.inactive_effective_month&&<span style={{fontSize:10,color:"#777777"}}>from {emp.inactive_effective_month}</span>}
                    </div>
                  </td>
              ...
              {sorted.length>0&&<tr style={{background:"#1E1E1E",borderTop:"2px solid #cbd5e1"}}>
                <td colSpan={3} style={{padding:"9px 13px",fontWeight:700,fontSize:13,color:"#FFFFFF"}}>Total ({filtered.length} employees)</td>
                <td style={{padding:"9px 13px",textAlign:"right",fontWeight:700,fontSize:13,color:"#FFFFFF"}}>{SAR(totalCost)}</td>
                <td colSpan={3}/>
              </tr>}
            </tbody>
          </table>
          {sorted.length===0&&<div style={{textAlign:"center",padding:"40px",color:"#777777"}}><div style={{fontSize:36,marginBottom:10}}>👤</div><p style={{fontSize:14}}>No employees found</p></div>}
        </div>
      </Card>
      ...
      <Modal open={modalOpen} onClose={close} title={editing?"Edit Employee":"Add New Employee"}>
        <form onSubmit={handleSubmit}><div style={{display:"flex",flexDirection:"column",gap:13}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}><Avatar name={form.name||"?"} size={50}/><div style={{flex:1}}><Lbl>Profile Picture</Lbl><input type="file" accept="image/*" disabled style={{fontSize:11,color:"#777777"}}/></div></div>
      ...
      <Modal open={inactiveModal} onClose={()=>{setInactiveModal(false);setPendingInactive(null);}} title="Set Inactive Effective Month">
        <div style={{display:"flex",flexDirection:"column",gap:13}}>
          <p style={{margin:0,fontSize:13,color:"#777777"}}>From which month should this employee be considered inactive?</p>
...
function ClientsPage(){
  ...
  const statusStyle=s=>({
    Active:  {bg:"#d1fae5",color:"#22C55E"},
    Prospect:{bg:"#dbeafe",color:"#1DC99A"},
    Inactive:{bg:"#f1f5f9",color:"#777777"},
    Churned: {bg:"#fee2e2",color:"#EF4444"},
  }[s]||{bg:"#f1f5f9",color:"#777777"});
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10}}>
        <div>
          <h1 style={{fontSize:26,fontWeight:800,color:"#FFFFFF",margin:0}}>Clients</h1>
          <p style={{fontSize:13,color:"#777777",marginTop:3}}>Manage your client portfolio</p>
        </div>
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
        <div style={{position:"relative",flex:1,minWidth:180}}>
          <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#777777",fontSize:13}}>🔍</span>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search clients..." style={{width:"100%",padding:"8px 12px 8px 30px",border:"1px solid #262626",borderRadius:9,fontSize:13,outline:"none",background:"#161616",boxSizing:"border-box"}}/>
        </div>
      ...
      <Card style={{overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead>
              <tr>
                ...
                <th style={{padding:"9px 13px",fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626",textAlign:"left"}}>Contact</th>
                <th style={{padding:"9px 13px",fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626",textAlign:"left"}}>Contract</th>
                <SortTh k="status" sk={sk} sd={sd} onSort={sort} align="center">Status</SortTh>
                <th style={{padding:"9px 13px",fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626",textAlign:"right"}}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {sorted.map((client,idx)=>{
                ...
                return(
                  <tr key={client.id} style={{borderBottom:"1px solid #1E1E1E",background:idx%2===0?"#161616":"#111111"}}>
                    ...
                    <td style={{padding:"12px 13px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                        ...
                        <div>
                          <p style={{margin:0,fontWeight:600,fontSize:13,color:"#FFFFFF"}}>{client.name}</p>
                          <p style={{margin:"1px 0 0",fontSize:11,color:"#777777"}}>{client.industry||"—"}</p>
                        </div>
                      </div>
                    </td>
                    {/* Contact info */}
                    <td style={{padding:"12px 13px"}}>
                      <div style={{display:"flex",flexDirection:"column",gap:2}}>
                        {client.contact_person&&<p style={{margin:0,fontSize:13,fontWeight:500,color:"#FFFFFF"}}>{client.contact_person}</p>}
                        {client.contact_person_designation&&<p style={{margin:0,fontSize:11,color:"#777777"}}>{client.contact_person_designation}</p>}
                        {client.contact_email&&<p style={{margin:"2px 0 0",fontSize:11,color:"#777777",display:"flex",alignItems:"center",gap:3}}>✉ {client.contact_email}</p>}
                        {client.contact_phone&&<p style={{margin:0,fontSize:11,color:"#777777",display:"flex",alignItems:"center",gap:3}}>📞 {client.contact_phone}</p>}
                      </div>
                    </td>
                    {/* Active contract */}
                    <td style={{padding:"12px 13px"}}>
                      {contract?(
                        <div>
                          <p style={{margin:0,fontWeight:600,fontSize:13,color:"#FFFFFF"}}>SAR {Math.round(contract.cv/contract.tm).toLocaleString("en-US")}/mo</p>
                          <p style={{margin:"1px 0 0",fontSize:11,color:"#777777"}}>{contract.tm} months</p>
                        </div>
                      ):(
          ...
          {sorted.length===0&&(
            <div style={{textAlign:"center",padding:"48px 24px",color:"#777777"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:13}}>
            ...
            <div><Lbl>Notes</Lbl>
              <textarea value={form.notes} onChange={e=>upd("notes",e.target.value)} placeholder="Additional notes..." rows={3}
                style={{width:"100%",padding:"8px 11px",border:"1px solid #262626",borderRadius:8,fontSize:13,color:"#FFFFFF",outline:"none",resize:"vertical",boxSizing:"border-box"}}/>
            </div>
...
function ContractSearchSelect({contracts,value,onChange}){
  ...
  return(
    <div style={{border:"1px solid #262626",borderRadius:8,overflow:"hidden",background:"#161616"}}>
      <div style={{display:"flex",borderBottom:"1px solid #1E1E1E"}}>
        {["all","Retainer","Project","Adhoc"].map(cat=>{
          const act=catF===cat; const c=catColors[cat];
          return <button key={cat} type="button" onClick={()=>setCatF(cat)} style={{flex:1,padding:"4px 2px",fontSize:10,fontWeight:600,border:"none",background:act?c.b:"#161616",color:act?c.a:"#777777",cursor:"pointer",borderBottom:act?`2px solid ${c.a}`:"2px solid transparent"}}>{cat==="all"?"All":cat}</button>;
        })}
      </div>
      <div style={{display:"flex",alignItems:"center",gap:5,padding:"5px 8px",borderBottom:"1px solid #1E1E1E"}}>
        <span style={{fontSize:11,color:"#777777"}}>🔍</span>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search..." style={{flex:1,fontSize:11,outline:"none",border:"none",background:"transparent",color:"#FFFFFF"}}/>
        {selected&&<button type="button" onClick={()=>onChange("")} style={{fontSize:11,color:"#777777",border:"none",background:"none",cursor:"pointer"}}>✕</button>}
      </div>
      {selected&&<div style={{padding:"3px 8px",background:"#0d0d1f",fontSize:11,fontWeight:600,color:"#1DC99A",borderBottom:"1px solid #e0e7ff"}}>✓ {selected.contract_number} – {selected.cn}</div>}
      <div style={{maxHeight:110,overflowY:"auto"}}>
        {filtered.length===0
          ?<p style={{padding:"6px 8px",fontSize:11,color:"#777777",textAlign:"center"}}>No contracts</p>
          :filtered.map(c=>(
            <button key={c.id} type="button" onClick={()=>{onChange(c.id);setSearch("");}}
              style={{width:"100%",textAlign:"left",padding:"5px 8px",fontSize:11,border:"none",background:c.id===value?"#1E1E1E":"#161616",cursor:"pointer",borderBottom:"1px solid #f8fafc",display:"flex",gap:6,alignItems:"baseline"}}>
              <span style={{fontFamily:"monospace",color:"#777777",flexShrink:0}}>{c.contract_number}</span>
              <span style={{color:"#FFFFFF",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.cn}</span>
...
function ContractsPage(){
  ...
  const budgetSummary=[
    {label:"Client Servicing",key:"budget_client_servicing",bg:"#eff6ff",border:"#bfdbfe",color:"#1DC99A"},
    {label:"Production",      key:"budget_production",       bg:"#f5f3ff",border:"#ddd6fe",color:"#6d28d9"},
    {label:"Creative",        key:"budget_creative",         bg:"#f0fdf4",border:"#bbf7d0",color:"#22C55E"},
  ...
  const catStyle=c=>({Retainer:{bg:"#dbeafe",col:"#2563eb"},Project:{bg:"#f3e8ff",col:"#1DC99A"},Adhoc:{bg:"#fef9c3",col:"#d97706"}}[c]||{bg:"#f1f5f9",col:"#475569"});

  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      {/* Header */}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10}}>
        <div><h1 style={{fontSize:26,fontWeight:800,color:"#FFFFFF",margin:0}}>Contracts</h1><p style={{fontSize:13,color:"#777777",marginTop:3}}>Manage client contracts and retainers</p></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        {budgetSummary.map(w=>(
          <Card key={w.label} style={{background:w.bg,borderColor:w.border,padding:16}}>
            ...
            <p style={{fontSize:10,color:"#777777",margin:0}}>across all contracts</p>
          </Card>
        ))}
      </div>
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
        <div style={{position:"relative",flex:1,minWidth:180}}>
          <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#777777",fontSize:13}}>🔍</span>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search contracts..." style={{width:"100%",padding:"8px 12px 8px 30px",border:"1px solid #262626",borderRadius:9,fontSize:13,outline:"none",background:"#161616",boxSizing:"border-box"}}/>
        </div>
        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            <thead><tr>
              ...
              <th style={{padding:"9px 13px",fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626"}}>Category</th>
              <SortTh k="contract_value"  sk={sk} sd={sd} onSort={sort} align="right">Contract Value</SortTh>
              <th style={{padding:"9px 13px",fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626",textAlign:"right"}}>Monthly Retainer</th>
              <SortTh k="start_date"      sk={sk} sd={sd} onSort={sort}>Duration</SortTh>
              <th style={{padding:"9px 13px",fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626",textAlign:"center"}}>Renewal</th>
              <SortTh k="status"          sk={sk} sd={sd} onSort={sort} align="center">Status</SortTh>
              <th style={{padding:"9px 13px",fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626",textAlign:"right"}}>Actions</th>
            </tr></thead>
            <tbody>
              {sorted.map((c,idx)=>{
                ...
                return(
                  <tr key={c.id} style={{borderBottom:"1px solid #1E1E1E",background:rowBg}}>
                    <td style={{padding:"11px 13px"}}><span style={{background:"#1E1E1E",color:"#B3B3B3",padding:"2px 8px",borderRadius:5,fontSize:11,fontWeight:600,fontFamily:"monospace"}}>{c.contract_number||"—"}</span></td>
                    <td style={{padding:"11px 13px",fontWeight:600,fontSize:13,color:"#FFFFFF"}}>{c.client_name}</td>
                    <td style={{padding:"11px 13px"}}><Bdg bg={cs.bg} color={cs.col}>{c.contract_category||"Retainer"}</Bdg></td>
                    <td style={{padding:"11px 13px",textAlign:"right",fontWeight:700,fontSize:13,color:"#FFFFFF"}}>SAR {c.contract_value.toLocaleString("en-US")}</td>
                    <td style={{padding:"11px 13px",textAlign:"right",fontSize:13,color:"#B3B3B3"}}>SAR {monthly.toLocaleString("en-US")}</td>
                    <td style={{padding:"11px 13px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:6}}>
                        <span style={{fontSize:12,color:"#777777"}}>📅</span>
                        <div>
                          <p style={{margin:0,fontSize:12,color:"#FFFFFF"}}>{fmtDateShort(c.start_date)}</p>
                          <p style={{margin:"1px 0 0",fontSize:11,color:"#777777"}}>to {fmtDateShort(c.end_date)}</p>
                        </div>
                      </div>
                    </td>
                    <td style={{padding:"11px 13px",textAlign:"center"}}>
                    ...
                    <td style={{padding:"11px 13px",textAlign:"center"}}>
                      <Bdg bg={expired?"#fee2e2":"#d1fae5"} color={expired?"#EF4444":"#22C55E"}>{expired?"Expired":"Active"}</Bdg>
                    </td>
              ...
              {/* Totals row */}
              {sorted.length>0&&(
                <tr style={{background:"#1E1E1E",borderTop:"2px solid #cbd5e1"}}>
                  <td colSpan={3} style={{padding:"9px 13px",fontWeight:700,fontSize:13,color:"#FFFFFF"}}>TOTAL</td>
                  <td style={{padding:"9px 13px",textAlign:"right",fontWeight:700,fontSize:13,color:"#FFFFFF"}}>SAR {sorted.reduce((s,c)=>s+c.contract_value,0).toLocaleString("en-US")}</td>
                  <td style={{padding:"9px 13px",textAlign:"right",fontWeight:700,fontSize:13,color:"#FFFFFF"}}>SAR {sorted.reduce((s,c)=>s+Math.round(c.contract_value/c.tenure_months),0).toLocaleString("en-US")}</td>
                  <td colSpan={4}/>
                </tr>
              )}
            </tbody>
          </table>
          {sorted.length===0&&<div style={{textAlign:"center",padding:"48px 24px",color:"#777777"}}><div style={{fontSize:40,marginBottom:12}}>📄</div><p style={{fontSize:14}}>No contracts found</p></div>}
        </div>
      </Card>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{display:"flex",flexDirection:"column",gap:13}}>
            {editing&&form.contract_number&&(
              <div style={{padding:"8px 12px",background:"#1E1E1E",borderRadius:8}}><p style={{margin:0,fontSize:12,color:"#B3B3B3"}}>Contract ID: <strong style={{color:"#FFFFFF"}}>{form.contract_number}</strong></p></div>
            )}
            <div><Lbl>Client *</Lbl>
            ...
            {form.contract_value&&form.tenure_months>0&&(
              <div style={{padding:"8px 12px",background:"#161616",borderRadius:8}}>
                <p style={{margin:0,fontSize:12,color:"#B3B3B3"}}>Monthly Retainer: <strong style={{color:"#FFFFFF"}}>SAR {Math.round(parseFloat(form.contract_value)/form.tenure_months).toLocaleString("en-US")}</strong></p>
              </div>
            )}
            {/* Dept Budget Allocation */}
            <div style={{border:"1px solid #262626",borderRadius:10,padding:14,background:"#161616"}}>
              <p style={{margin:"0 0 10px",fontSize:12,fontWeight:700,color:"#B3B3B3"}}>Budget Allocation by Department</p>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              ...
              <div style={{marginTop:10,padding:"8px 12px",borderRadius:8,background:allocMatch?"#d1fae5":allocOver?"#fee2e2":"#fef9c3",border:`1px solid ${allocMatch?"#a7f3d0":allocOver?"#fecaca":"#fde68a"}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                <span style={{fontSize:12,fontWeight:600,color:"#B3B3B3"}}>Total Allocated:</span>
                <span style={{fontSize:13,fontWeight:700,color:allocMatch?"#22C55E":allocOver?"#EF4444":"#d97706"}}>
                  SAR {totalAlloc.toLocaleString("en-US")}
                  {formCV>0&&<span style={{fontSize:11,fontWeight:400,color:"#777777",marginLeft:6}}>/ SAR {formCV.toLocaleString("en-US")}</span>}
                </span>
              </div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            ...
            <div><Lbl>Contract PDF</Lbl><Inp type="file" accept="application/pdf" disabled style={{color:"#777777"}}/></div>
            <div><Lbl>Notes</Lbl>
              <textarea value={form.notes} onChange={e=>upd("notes",e.target.value)} placeholder="Contract notes..." rows={2} style={{width:"100%",padding:"8px 11px",border:"1px solid #262626",borderRadius:8,fontSize:13,color:"#FFFFFF",outline:"none",resize:"vertical",boxSizing:"border-box"}}/>
            </div>
            <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:4}}>
              <Btn variant="outline" onClick={close}>Cancel</Btn>
              <Btn variant="primary" type="submit">{editing?"Update":"Create"}</Btn>
...
function AllocationsPage(){
  ...
  const statusBadge=s=>s==="Assigned"?{bg:"#d1fae5",col:"#22C55E"}:{bg:"#f1f5f9",col:"#64748b"};
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      {/* Header */}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10}}>
        <div><h1 style={{fontSize:26,fontWeight:800,color:"#FFFFFF",margin:0}}>Team Allocations</h1><p style={{fontSize:13,color:"#777777",marginTop:3}}>Assign employees to client projects</p></div>
        <Btn variant="primary" onClick={openAdd}>＋ Add Allocation</Btn>
      </div>

      {/* Chart month selector */}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <p style={{margin:0,fontSize:13,fontWeight:600,color:"#777777"}}>Allocation Overview</p>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <Card style={{padding:18}}>
          <p style={{margin:"0 0 10px",fontWeight:700,fontSize:13,color:"#FFFFFF"}}>🥧 Hours Utilization</p>
          {totalCap>0?(
            <div style={{display:"flex",alignItems:"center",gap:16}}>
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <PieChart>
                    <Pie data={pieData} cx="50%" cy="50%" innerRadius={45} outerRadius={72} dataKey="hours" labelLine={false}>
                      <Cell fill="#1DC99A"/><Cell fill="#e2e8f0"/>
                    </Pie>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10,minWidth:120}}>
                <div><div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}><div style={{width:10,height:10,borderRadius:"50%",background:"#1DC99A"}}/><span style={{fontSize:11,color:"#777777"}}>Utilized</span></div><p style={{margin:0,fontSize:17,fontWeight:800,color:"#FFFFFF"}}>{utilizedHours}h</p><p style={{margin:0,fontSize:10,color:"#777777"}}>{totalCap>0?((utilizedHours/totalCap)*100).toFixed(0):0}% of capacity</p></div>
                <div><div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}><div style={{width:10,height:10,borderRadius:"50%",background:"#262626"}}/><span style={{fontSize:11,color:"#777777"}}>Available</span></div><p style={{margin:0,fontSize:17,fontWeight:800,color:"#FFFFFF"}}>{availHours}h</p><p style={{margin:0,fontSize:10,color:"#777777"}}>of {totalCap}h total</p></div>
                <div style={{paddingTop:7,borderTop:"1px solid #1E1E1E"}}><p style={{margin:"0 0 1px",fontSize:10,color:"#777777"}}>Resources</p><p style={{margin:0,fontSize:17,fontWeight:800,color:"#FFFFFF"}}>{activeEmps.length}</p><p style={{margin:0,fontSize:10,color:"#777777"}}>active employees</p></div>
              </div>
            </div>
          ):<div style={{height:180,display:"flex",alignItems:"center",justifyContent:"center",color:"#777777",fontSize:13}}>No allocation data</div>}
        </Card>

        <Card style={{padding:18}}>
          <p style={{margin:"0 0 10px",fontWeight:700,fontSize:13,color:"#FFFFFF"}}>👥 Top Clients by Hours</p>
          {clientChartData.length>0?(
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={clientChartData} margin={{top:5,right:5,left:0,bottom:36}}>
                ...
                <Bar dataKey="hours" fill="#1DC99A" radius={[4,4,0,0]}/>
              </BarChart>
            </ResponsiveContainer>
          ):<div style={{height:180,display:"flex",alignItems:"center",justifyContent:"center",color:"#777777",fontSize:13}}>No allocation data</div>}
        </Card>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
        {EMPLOYEES_INIT.filter(e=>isEmpActiveForMonth(e,chartMonth)).map(emp=>{
          ...
          const clr=ov?"#EF4444":ok?"#22C55E":"#d97706";
          return(
            <Card key={emp.id} style={{background:bg2,borderColor:border,padding:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:7}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <div style={{width:30,height:30,borderRadius:8,background:"#1E1E1E",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:clr,flexShrink:0}}>{emp.name.slice(0,2).toUpperCase()}</div>
                  <div><p style={{margin:0,fontWeight:600,fontSize:12,color:"#FFFFFF"}}>{emp.name}</p><p style={{margin:0,fontSize:10,color:"#777777"}}>{emp.designation}</p></div>
                </div>
                {ov&&<span style={{fontSize:12}}>⚠️</span>}
              </div>
              <PBar val={u.percentage} color={clr}/>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#777777",marginTop:3}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        <div style={{position:"relative",flex:1,minWidth:155}}>
          <span style={{position:"absolute",left:9,top:"50%",transform:"translateY(-50%)",color:"#777777",fontSize:12}}>🔍</span>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search allocations..." style={{width:"100%",padding:"7px 10px 7px 27px",border:"1px solid #262626",borderRadius:8,fontSize:13,outline:"none",background:"#161616",boxSizing:"border-box"}}/>
        </div>
      ...
      <Card style={{overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>
              {sorted.map((a,idx)=>{
                ...
                return(
                  <tr key={a.id} style={{borderBottom:"1px solid #1E1E1E",background:idx%2===0?"#161616":"#111111"}}>
                    <td style={{padding:"11px 13px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                        ...
                        <div><p style={{margin:0,fontWeight:600,fontSize:13,color:"#FFFFFF"}}>{a.employee_name}</p><p style={{margin:"1px 0 0",fontSize:10,color:"#777777"}}>{emp?.department?.replace(" Department","")}</p></div>
                      </div>
                    </td>
                    <td style={{padding:"11px 13px",fontSize:13,color:"#FFFFFF"}}>{a.client_name}</td>
                    <td style={{padding:"11px 13px",textAlign:"center"}}><Bdg bg="#f1f5f9" color="#475569">🕐 {a.allocated_hours} hrs</Bdg></td>
                    <td style={{padding:"11px 13px",textAlign:"center",fontSize:12,color:"#777777"}}>{a.month||"—"}</td>
          ...
          {sorted.length===0&&<div style={{textAlign:"center",padding:"48px",color:"#777777"}}><div style={{fontSize:40,marginBottom:12}}>👥</div><p style={{fontSize:14}}>No allocations found</p></div>}
        </div>
      </Card>
      ...
      <Modal open={modalOpen} onClose={closeModal} title={editing?"Edit Allocation":"Add New Allocation"}>
        ...
        {editing&&(
          <form onSubmit={handleEditSubmit}>
            <div style={{display:"flex",flexDirection:"column",gap:13}}>
              <div style={{padding:"8px 12px",background:"#161616",borderRadius:8}}>
                <p style={{margin:0,fontSize:12,color:"#B3B3B3"}}>Employee: <strong style={{color:"#FFFFFF"}}>{editing.employee_name}</strong></p>
                <p style={{margin:"2px 0 0",fontSize:12,color:"#B3B3B3"}}>Client: <strong style={{color:"#FFFFFF"}}>{editing.client_name}</strong></p>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                <div>
                  <Lbl>Hours/Month *</Lbl>
                  <Inp type="number" min="0" value={editForm.allocated_hours} onChange={e=>setEditForm(p=>({...p,allocated_hours:e.target.value}))} required/>
                  <p style={{margin:"3px 0 0",fontSize:10,color:"#777777"}}>{getRemainingHours(editing.employee_id,editForm.month,editing.id)+editing.allocated_hours}h available</p>
                </div>
        ...
        {!editing&&(
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              {[["1","Select"],["2","Configure"]].map(([num,label],i)=>{
                ...
                return(
                  <div key={num} style={{display:"flex",alignItems:"center",gap:8}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,padding:"4px 12px",borderRadius:999,background:act?"#1DC99A":"#1E1E1E",color:act?"#000":"#777777",fontSize:12,fontWeight:600}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:13}}>
                ...
                <div>
                  ...
                  <div style={{position:"relative",marginBottom:6}}>
                    <span style={{position:"absolute",left:9,top:"50%",transform:"translateY(-50%)",color:"#777777",fontSize:12}}>🔍</span>
                    <input value={empSearch} onChange={e=>setEmpSearch(e.target.value)} placeholder="Search employees..." style={{width:"100%",padding:"7px 10px 7px 27px",border:"1px solid #262626",borderRadius:8,fontSize:13,outline:"none",background:"#161616",boxSizing:"border-box"}}/>
                  </div>
                  <div style={{border:"1px solid #262626",borderRadius:9,maxHeight:210,overflowY:"auto"}}>
                    {EMPLOYEES_INIT
                      ...
                      const tc=avail<=0?"#EF4444":pct>=70?"#22C55E":"#d97706";
                      return(
                        <label key={emp.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",cursor:"pointer",background:isSel?"#1E1E1E":"#161616",borderBottom:"1px solid #262626"}}>
                          ...
                          <div style={{flex:1}}><p style={{margin:0,fontWeight:600,fontSize:12,color:"#FFFFFF"}}>{emp.name}</p><p style={{margin:0,fontSize:10,color:"#777777"}}>{emp.department?.replace(" Department","")}</p></div>
                          <Bdg bg={bc} color={tc}>{avail}h free</Bdg>
                        </label>
                      );
                    })}
                  </div>
                  {selEmpIds.length>0&&<p style={{margin:"4px 0 0",fontSize:12,color:"#777777"}}>{selEmpIds.length} employee(s) selected</p>}
                </div>
            ...
            {formStep===2&&(
              <div style={{display:"flex",flexDirection:"column",gap:13}}>
                <div style={{padding:"7px 12px",background:"#161616",borderRadius:8}}><p style={{margin:0,fontSize:12,color:"#B3B3B3"}}>Month: <strong style={{color:"#FFFFFF"}}>{ALLOC_MONTHS.find(m=>m.v===selMonth)?.l}</strong></p></div>
                <div style={{display:"flex",flexDirection:"column",gap:10,maxHeight:330,overflowY:"auto"}}>
                  {selEmpIds.map(eid=>{
                    ...
                    return(
                      <div key={eid} style={{border:"1px solid #262626",borderRadius:10,padding:13,background:"#111111"}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:9}}>
                          <div style={{display:"flex",alignItems:"center",gap:7}}><span style={{fontSize:13}}>👤</span><span style={{fontWeight:600,fontSize:13,color:"#FFFFFF"}}>{emp?.name}</span></div>
                        ...
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                          <div>
                            ...
                            <p style={{margin:"2px 0 0",fontSize:10,color:"#777777"}}>{rem}h available</p>
                          </div>
                  ...
                  {selEmpIds.length===0&&<p style={{textAlign:"center",color:"#777777",fontSize:13,padding:"12px 0"}}>No employees selected — go back to add some.</p>}
                </div>
      ...
      <Modal open={confirmOpen} onClose={()=>setConfirmOpen(false)} title="Confirm Allocations">
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <p style={{margin:0,fontSize:13,color:"#B3B3B3"}}>Create <strong>{selEmpIds.filter(id=>empAllocs[id]?.client_id&&parseFloat(empAllocs[id]?.hours)>0).length}</strong> allocation(s) for <strong>{ALLOC_MONTHS.find(m=>m.v===selMonth)?.l}</strong>?</p>
...
function ReportsPage(){
  ...
  const marginBadge=(m)=>{
    ...
    const col=m<0?"#EF4444":m<20?"#d97706":"#22C55E";
    return <Bdg bg={bg} color={col}>{pct(m)}</Bdg>;
  };
  const riskBadge=(level)=>{
    const s={High:{bg:"#ef4444",col:"#fff"},Medium:{bg:"#f59e0b",col:"#fff"},Low:{bg:"#1DC99A",col:"#fff"}}[level]||{bg:"#e2e8f0",col:"#64748b"};
  ...
  // ── Table header style ───────────────────────────────────────────────────────
  const TH=({children,align="left"})=><th style={{padding:"8px 12px",textAlign:align,fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626",whiteSpace:"nowrap"}}>{children}</th>;
  const TDark=({children,align="left"})=><th style={{padding:"8px 12px",textAlign:align,fontSize:11,fontWeight:600,color:"#fff",background:"#161616",borderBottom:"1px solid #334155",whiteSpace:"nowrap"}}>{children}</th>;
  const TD=({children,align="left",style={}})=><td style={{padding:"8px 12px",textAlign:align,fontSize:13,borderBottom:"1px solid #1E1E1E",...style}}>{children}</td>;

  // ── Sub-tab bar ──────────────────────────────────────────────────────────────
  const SubTabs=({tabs,active,onChange})=>(
    <div style={{display:"flex",flexWrap:"wrap",gap:4,background:"#1E1E1E",borderRadius:10,padding:4,marginBottom:18}}>
      {tabs.map(t=>(
        <button key={t.id} onClick={()=>onChange(t.id)}
          style={{padding:"7px 14px",borderRadius:8,border:"none",background:active===t.id?"#1E1E1E":"transparent",fontWeight:active===t.id?600:500,fontSize:12,color:active===t.id?"#FFFFFF":"#777777",cursor:"pointer",boxShadow:active===t.id?"0 1px 3px rgba(0,0,0,.1)":"none"}}>
          {t.label}
        </button>
      ))}
    </div>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      {/* Header */}
      <div>
        <h1 style={{fontSize:26,fontWeight:800,color:"#FFFFFF",margin:0}}>Reports</h1>
        <p style={{fontSize:13,color:"#777777",marginTop:3}}>Detailed analysis and insights</p>
      </div>

      {/* Top section tabs */}
      <div style={{display:"flex",gap:4,background:"#1E1E1E",borderRadius:12,padding:4,maxWidth:420}}>
        {[["charts","📈 Charts & Graphs"],["custom","📋 Custom Reports"]].map(([v,l])=>(
          <button key={v} onClick={()=>setSection(v)} style={{flex:1,padding:"9px 14px",borderRadius:9,border:"none",background:section===v?"#1E1E1E":"transparent",fontWeight:section===v?700:500,fontSize:13,color:section===v?"#FFFFFF":"#777777",cursor:"pointer",boxShadow:section===v?"0 1px 3px rgba(0,0,0,.1)":"none"}}>{l}</button>
        ))}
      </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:0}}>
          ...
          {chartTab==="profit-by-client"&&(
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontSize:12,color:"#777777"}}>Month:</span>
              ...
              <Card style={{padding:18}}>
                <p style={{margin:"0 0 14px",fontWeight:700,fontSize:14,color:"#FFFFFF"}}>Profit by Client</p>
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={R.profitByClient.slice(0,8)} margin={{top:10,right:20,left:10,bottom:50}}>
                    ...
                    <Tooltip formatter={v=>SAR(v)} contentStyle={{borderRadius:8,border:"1px solid #262626",background:"#161616",color:"#FFFFFF"}}/>
                    <Legend wrapperStyle={{fontSize:11,color:"#777777"}}/>
                    <Bar dataKey="monthlyRetainer" name="Monthly Retainer" fill="#1DC99A" radius={[3,3,0,0]}/>
                    <Bar dataKey="resourceCost"    name="Resource Cost"    fill="#f59e0b" radius={[3,3,0,0]}/>
                    <Bar dataKey="monthlyProfit"   name="Monthly Profit"   fill="#1DC99A" radius={[3,3,0,0]}/>
                  </BarChart>
                </ResponsiveContainer>
                  ...
                  <table style={{width:"100%",borderCollapse:"collapse"}}>
                    ...
                    <tbody>{R.profitByClient.map(c=>(
                      <tr key={c.id}>
                        ...
                        <TD align="right" style={{fontWeight:700,color:c.monthlyProfit>=0?"#22C55E":"#EF4444"}}>{SAR(c.monthlyProfit)}</TD>
          ...
          {chartTab==="profit-by-department"&&(
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              <Card style={{padding:18}}>
                <p style={{margin:"0 0 14px",fontWeight:700,fontSize:14,color:"#FFFFFF"}}>Profit by Department vs Contract Budget</p>
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={R.profitByDepartment} margin={{top:10,right:20,left:10,bottom:50}}>
                    ...
                    <Tooltip formatter={v=>SAR(v)} contentStyle={{borderRadius:8,border:"1px solid #262626",background:"#161616",color:"#FFFFFF"}}/>
                    <Legend wrapperStyle={{fontSize:11,color:"#777777"}}/>
                    <Bar dataKey="budget" name="Budget"      fill="#1DC99A" radius={[3,3,0,0]}/>
                    <Bar dataKey="cost"   name="Actual Cost" fill="#f59e0b" radius={[3,3,0,0]}/>
                    <Bar dataKey="profit" name="Profit"      fill="#1DC99A" radius={[3,3,0,0]}/>
                  </BarChart>
                </ResponsiveContainer>
                <div style={{overflowX:"auto",marginTop:16}}>
                  <table style={{width:"100%",borderCollapse:"collapse"}}>
                    ...
                    <tbody>{R.profitByDepartment.map(d=>(
                      <tr key={d.name}>
                        ...
                        <TD align="right" style={{fontWeight:700,color:d.profit>=0?"#22C55E":"#EF4444"}}>{SAR(d.profit)}</TD>
              ...
              {R.resourceByDepartment.filter(d=>selDept==="all"||d.department===selDept).map(dept=>(
                <Card key={dept.department} style={{padding:18}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                    <p style={{margin:0,fontWeight:700,fontSize:14,color:"#FFFFFF"}}>{dept.department} Department</p>
                    <span style={{fontSize:12,color:"#777777"}}>{dept.employeeCount} employees · {dept.totalHours}h · {dept.avgUtilization.toFixed(0)}% utilized</span>
                  </div>
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={dept.clientBreakdown} margin={{top:5,right:10,left:5,bottom:36}}>
                      ...
                      <YAxis tick={{fontSize:9,fill:"#64748b"}}/>
                      <Tooltip formatter={(v,n)=>n==="hours"?`${v}h`:SAR(v)} contentStyle={{borderRadius:8,border:"1px solid #262626",background:"#161616",color:"#FFFFFF"}}/>
                      <Legend wrapperStyle={{fontSize:10,color:"#777777"}}/>
                      <Bar dataKey="hours" name="Hours" fill="#1DC99A" radius={[3,3,0,0]}/>
                      <Bar dataKey="cost"  name="Cost"  fill="#1DC99A" radius={[3,3,0,0]}/>
                    </BarChart>
                  </ResponsiveContainer>
                  <table style={{width:"100%",borderCollapse:"collapse",marginTop:12}}>
                    ...
                    <tbody>
                      {dept.clientBreakdown.map((c,i)=>(
                        <tr key={i}><TD><strong>{c.client}</strong></TD><TD align="center">{c.hours}h</TD><TD align="right" style={{color:"#22C55E"}}>{SAR(c.cost)}</TD></tr>
                      ))}
          ...
          {chartTab==="cash-flow"&&(
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              <Card style={{padding:18}}>
                <p style={{margin:"0 0 14px",fontWeight:700,fontSize:14,color:"#FFFFFF"}}>12-Month Cash Flow Forecast</p>
                <ResponsiveContainer width="100%" height={260}>
                  <LineChart data={R.cashFlow} margin={{top:10,right:20,left:10,bottom:5}}>
                    ...
                    <Tooltip formatter={v=>SAR(v)} contentStyle={{borderRadius:8,border:"1px solid #262626",background:"#161616",color:"#FFFFFF"}}/>
                    <Legend wrapperStyle={{fontSize:11,color:"#777777"}}/>
                    <Line type="monotone" dataKey="expectedRevenue" name="Revenue"       stroke="#1DC99A" strokeWidth={3}/>
                    <Line type="monotone" dataKey="expectedCost"    name="Cost"          stroke="#f59e0b" strokeWidth={3}/>
                    <Line type="monotone" dataKey="netCashFlow"     name="Net Cash Flow" stroke="#1DC99A" strokeWidth={2} strokeDasharray="5 5"/>
                  </LineChart>
                </ResponsiveContainer>
                <div style={{overflowX:"auto",marginTop:16}}>
                  <table style={{width:"100%",borderCollapse:"collapse"}}>
                    ...
                    <tbody>{R.cashFlow.map((r,i)=>(
                      <tr key={i}>
                        ...
                        <TD align="right" style={{color:"#22C55E"}}>{SAR(r.expectedRevenue)}</TD>
                        <TD align="right" style={{color:"#d97706"}}>{SAR(r.expectedCost)}</TD>
                        <TD align="right" style={{fontWeight:700,color:r.netCashFlow>=0?"#1DC99A":"#EF4444"}}>{SAR(r.netCashFlow)}</TD>
          ...
          {chartTab==="risk-analysis"&&(
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              <Card style={{padding:18}}>
                <p style={{margin:"0 0 14px",fontWeight:700,fontSize:14,color:"#FFFFFF"}}>Client Risk Analysis & Recommendations</p>
                <div style={{overflowX:"auto"}}>
                  <table style={{width:"100%",borderCollapse:"collapse"}}>
                    <thead><tr>
                    ...
                    <tbody>{R.clientRisk.map(c=>(
                      <tr key={c.id} style={{background:c.riskLevel==="High"?"#1a0808":c.riskLevel==="Medium"?"#1a1000":"#161616"}}>
                        <TD><strong>{c.name}</strong></TD>
                        <TD align="right">{SAR(c.contractValue)}</TD>
                        <TD align="right" style={{fontWeight:700,color:c.monthlyProfit>=0?"#22C55E":"#EF4444"}}>{SAR(c.monthlyProfit)}</TD>
                        <TD align="center">{marginBadge(c.marginPercent)}</TD>
                        ...
                        <TD style={{fontSize:12,color:"#B3B3B3"}}>{c.recommendation}</TD>
                      </tr>
                    ))}</tbody>
                  </table>
                </div>
          ...
          {chartTab==="monthly-closed"&&(
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              <Card style={{padding:18}}>
                <p style={{margin:"0 0 14px",fontWeight:700,fontSize:14,color:"#FFFFFF"}}>Historical Closed Months Performance</p>
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart data={R.closedMonths.map(m=>{
                    ...
                    <Tooltip formatter={v=>SAR(v)} contentStyle={{borderRadius:8,border:"1px solid #262626",background:"#161616",color:"#FFFFFF"}}/>
                    <Legend wrapperStyle={{fontSize:11,color:"#777777"}}/>
                    <Bar dataKey="retainer" name="Monthly Retainer" fill="#1DC99A" radius={[3,3,0,0]}/>
                    ...
                    <tbody>{R.closedMonths.flatMap(m=>
                      m.clients.filter(c=>selClosedClient==="all"||c.name===selClosedClient).map((c,i)=>(
                        <tr key={`${m.month}-${i}`}>
                          ...
                          <TD align="right" style={{fontWeight:700,color:c.profit>=0?"#22C55E":"#EF4444"}}>{SAR(c.profit)}</TD>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
                  <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                      <span style={{fontSize:12,color:"#777777"}}>Category:</span>
                ...
                <Card style={{overflow:"hidden"}}>
                  <div style={{padding:"14px 18px",borderBottom:"1px solid #1E1E1E"}}><p style={{margin:0,fontWeight:700,fontSize:14,color:"#FFFFFF"}}>Revenue & Profit by Contract</p></div>
                  <div style={{overflowX:"auto"}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                      <thead><tr>
                        {["Month","Client Name","Contract #","Contract Value","Start Date","End Date","Monthly Revenue","Hours","Resource Cost","Profit","Margin %","Status"].map((h,i)=>(
                          <th key={h} style={{padding:"8px 10px",textAlign:i>=3&&i<=9?"right":i>=10?"center":"left",fontSize:11,fontWeight:600,color:"#fff",background:"#161616",borderBottom:"1px solid #334155",whiteSpace:"nowrap"}}>{h}</th>
                        ))}
                      </tr></thead>
                      <tbody>
                        {rows.length===0
                          ?<tr><td colSpan={12} style={{textAlign:"center",padding:40,color:"#777777"}}>No closed month data available yet</td></tr>
                          :rows.map((r,i)=>(
                            <tr key={i} style={{background:i%2===0?"#161616":"#111111"}}>
                              <td style={{padding:"7px 10px",fontWeight:600,fontSize:12,whiteSpace:"nowrap",borderBottom:"1px solid #1E1E1E"}}>{r.month}</td>
                              <td style={{padding:"7px 10px",fontSize:12,whiteSpace:"nowrap",borderBottom:"1px solid #1E1E1E"}}>{r.client_name}</td>
                              <td style={{padding:"7px 10px",fontSize:11,fontFamily:"monospace",whiteSpace:"nowrap",borderBottom:"1px solid #1E1E1E"}}>{r.contract_number}</td>
                              <td style={{padding:"7px 10px",textAlign:"right",fontSize:12,whiteSpace:"nowrap",borderBottom:"1px solid #1E1E1E"}}>{r.contract_value.toLocaleString("en-US",{minimumFractionDigits:2})}</td>
                              <td style={{padding:"7px 10px",fontSize:12,whiteSpace:"nowrap",borderBottom:"1px solid #1E1E1E"}}>{r.start_date}</td>
                              <td style={{padding:"7px 10px",fontSize:12,whiteSpace:"nowrap",borderBottom:"1px solid #1E1E1E"}}>{r.end_date}</td>
                              <td style={{padding:"7px 10px",textAlign:"right",fontSize:12,fontWeight:600,color:"#22C55E",whiteSpace:"nowrap",borderBottom:"1px solid #1E1E1E"}}>{r.monthly_retainer.toLocaleString("en-US",{minimumFractionDigits:2})}</td>
                              <td style={{padding:"7px 10px",textAlign:"center",fontSize:12,borderBottom:"1px solid #1E1E1E"}}>{r.allocated_hours}h</td>
                              <td style={{padding:"7px 10px",textAlign:"right",fontSize:12,color:"#d97706",whiteSpace:"nowrap",borderBottom:"1px solid #1E1E1E"}}>{r.resource_cost.toLocaleString("en-US",{minimumFractionDigits:2})}</td>
                              <td style={{padding:"7px 10px",textAlign:"right",fontSize:12,fontWeight:700,color:r.profit>=0?"#22C55E":"#EF4444",whiteSpace:"nowrap",borderBottom:"1px solid #1E1E1E"}}>{r.profit.toLocaleString("en-US",{minimumFractionDigits:2})}</td>
                              <td style={{padding:"7px 10px",textAlign:"center",borderBottom:"1px solid #1E1E1E"}}>{marginBadge(r.margin)}</td>
                              <td style={{padding:"7px 10px",textAlign:"center",borderBottom:"1px solid #1E1E1E"}}><Bdg bg={r.status==="Active"?"#d1fae5":"#f1f5f9"} color={r.status==="Active"?"#22C55E":"#64748b"}>{r.status}</Bdg></td>
                            </tr>
                          ))
                        }
                        <tr style={{background:"#262626",fontWeight:700}}>
                          ...
                          <td style={{padding:"8px 10px",textAlign:"right",fontSize:12,color:"#22C55E"}}>{totRev.toLocaleString("en-US",{minimumFractionDigits:2})}</td>
                          ...
                          <td style={{padding:"8px 10px",textAlign:"right",fontSize:12,fontWeight:700,color:totProfit>=0?"#22C55E":"#EF4444"}}>{totProfit.toLocaleString("en-US",{minimumFractionDigits:2})}</td>
          ...
          {customTab==="employee-utilization"&&(()=>{
            ...
            return(
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                ...
                <Card style={{overflow:"hidden"}}>
                  <div style={{padding:"14px 18px",borderBottom:"1px solid #1E1E1E"}}><p style={{margin:0,fontWeight:700,fontSize:14,color:"#FFFFFF"}}>Employee Utilization — {fmtLong(selUtilMonth)}</p></div>
                  <div style={{overflowX:"auto"}}>
                    <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                      <thead><tr>
                        {["Employee Name","Department","Allocated Hrs","Available Hrs","Utilization %","Hourly Cost","Resource Cost","Contracts Worked On","Overallocated"].map((h,i)=>(
                          <th key={h} style={{padding:"8px 10px",textAlign:i>=2&&i<=6?"center":i===7?"left":"center",fontSize:11,fontWeight:600,color:"#fff",background:"#161616",borderBottom:"1px solid #334155",whiteSpace:"nowrap"}}>{h}</th>
                        ))}
                      </tr></thead>
                      <tbody>
                        {rows.map((r,i)=>{
                          const utilBg=r.pct>100?"#fee2e2":r.pct>=80?"#d1fae5":r.pct>=50?"#fef9c3":"#f1f5f9";
                          const utilCol=r.pct>100?"#EF4444":r.pct>=80?"#22C55E":r.pct>=50?"#d97706":"#64748b";
                          return(
                            <tr key={r.id} style={{background:r.over?"#1a0808":i%2===0?"#161616":"#111111"}}>
                              <td style={{padding:"7px 10px",fontWeight:600,fontSize:12,whiteSpace:"nowrap",borderBottom:"1px solid #1E1E1E"}}>{r.name}</td>
                              <td style={{padding:"7px 10px",fontSize:12,whiteSpace:"nowrap",borderBottom:"1px solid #1E1E1E"}}>{r.department?.replace(" Department","")}</td>
                              <td style={{padding:"7px 10px",textAlign:"center",borderBottom:"1px solid #1E1E1E"}}>{r.ah}h</td>
                              <td style={{padding:"7px 10px",textAlign:"center",borderBottom:"1px solid #1E1E1E"}}>{r.avail}h</td>
                              <td style={{padding:"7px 10px",textAlign:"center",borderBottom:"1px solid #1E1E1E"}}><Bdg bg={utilBg} color={utilCol}>{r.pct}%</Bdg></td>
                              <td style={{padding:"7px 10px",textAlign:"center",borderBottom:"1px solid #1E1E1E"}}>{SAR(r.hr)}/h</td>
                              <td style={{padding:"7px 10px",textAlign:"center",color:"#d97706",borderBottom:"1px solid #1E1E1E"}}>{r.cost.toLocaleString("en-US",{minimumFractionDigits:2})}</td>
                              <td style={{padding:"7px 10px",fontSize:11,color:"#B3B3B3",borderBottom:"1px solid #1E1E1E"}}>{r.contracts.length>0?r.contracts.join(", "):<span style={{color:"#777777"}}>None</span>}</td>
                              <td style={{padding:"7px 10px",textAlign:"center",borderBottom:"1px solid #1E1E1E"}}><Bdg bg={r.over?"#ef4444":"#f1f5f9"} color={r.over?"#fff":"#64748b"}>{r.over?"Yes":"No"}</Bdg></td>
                            </tr>
                          );
                        })}
                        <tr style={{background:"#262626",fontWeight:700}}>
          ...
          {customTab==="department-performance"&&(()=>{
            ...
            return(
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                ...
                <Card style={{padding:18}}>
                  <p style={{margin:"0 0 14px",fontWeight:700,fontSize:14,color:"#FFFFFF"}}>Department Performance — {fmtLong(selDeptMonth)}</p>
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={rows} margin={{top:5,right:20,left:10,bottom:5}}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0"/>
                      <XAxis dataKey="dept" tick={{fontSize:10,fill:"#64748b"}}/>
                      <YAxis tick={{fontSize:10,fill:"#64748b"}} tickFormatter={v=>`${v/1000}K`}/>
                      <Tooltip formatter={v=>typeof v==="number"&&v>100?SAR(v):`${v}`} contentStyle={{borderRadius:8,border:"1px solid #262626",background:"#161616",color:"#FFFFFF"}}/>
                      <Legend wrapperStyle={{fontSize:11,color:"#777777"}}/>
                      <Bar dataKey="totalBudget" name="Budget" fill="#1DC99A" radius={[3,3,0,0]}/>
                  ...
                  <div style={{overflowX:"auto",marginTop:16}}>
                    <table style={{width:"100%",borderCollapse:"collapse"}}>
                      <thead><tr>
                        <th style={{padding:"8px 10px",textAlign:"left",fontSize:11,fontWeight:600,color:"#fff",background:"#161616",borderBottom:"1px solid #334155"}}>Department</th>
                        {["Employees","Total Hours","Utilization %","Budget (SAR)","Actual Cost","Profit","Margin %"].map(h=>(
                          <th key={h} style={{padding:"8px 10px",textAlign:"center",fontSize:11,fontWeight:600,color:"#fff",background:"#161616",borderBottom:"1px solid #334155",whiteSpace:"nowrap"}}>{h}</th>
                        ))}
                      </tr></thead>
                      <tbody>{rows.map((r,i)=>(
                        <tr key={r.dept} style={{background:i%2===0?"#161616":"#111111"}}>
                          <td style={{padding:"8px 10px",fontWeight:600,fontSize:13,borderBottom:"1px solid #1E1E1E"}}>{r.dept}</td>
                          <td style={{padding:"8px 10px",textAlign:"center",fontSize:13,borderBottom:"1px solid #1E1E1E"}}>{r.empCount}</td>
                          <td style={{padding:"8px 10px",textAlign:"center",fontSize:13,borderBottom:"1px solid #1E1E1E"}}>{r.totalHours}h</td>
                          <td style={{padding:"8px 10px",textAlign:"center",borderBottom:"1px solid #1E1E1E"}}>{marginBadge(r.util)}</td>
                          <td style={{padding:"8px 10px",textAlign:"center",fontSize:13,borderBottom:"1px solid #1E1E1E"}}>{SAR(r.totalBudget)}</td>
                          <td style={{padding:"8px 10px",textAlign:"center",fontSize:13,color:"#d97706",borderBottom:"1px solid #1E1E1E"}}>{SAR(r.totalCost)}</td>
                          <td style={{padding:"8px 10px",textAlign:"center",fontSize:13,fontWeight:700,color:r.profit>=0?"#22C55E":"#EF4444",borderBottom:"1px solid #1E1E1E"}}>{SAR(r.profit)}</td>
                          <td style={{padding:"8px 10px",textAlign:"center",borderBottom:"1px solid #1E1E1E"}}>{marginBadge(r.margin)}</td>
                        </tr>
                      ))}</tbody>
                    </table>
                  </div>
...
function MonthlyClosePage(){
  ...
  const marginBadge=(v)=>{const bg=v<0?"#fee2e2":v<20?"#fef9c3":"#d1fae5";const col=v<0?"#EF4444":v<20?"#d97706":"#22C55E";return <Bdg bg={bg} color={col}>{v.toFixed(1)}%</Bdg>;};
  const TH=({children,align="left"})=><th style={{padding:"9px 13px",textAlign:align,fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626",whiteSpace:"nowrap"}}>{children}</th>;
  const TD=({children,align="left",style={}})=><td style={{padding:"9px 13px",textAlign:align,fontSize:13,borderBottom:"1px solid #1E1E1E",...style}}>{children}</td>;

  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      {/* Header */}
      <div>
        <h1 style={{fontSize:26,fontWeight:800,color:"#FFFFFF",margin:0}}>Monthly Financial Close</h1>
        <p style={{fontSize:13,color:"#777777",marginTop:3}}>Lock historical financial data for accurate reporting</p>
      </div>

      {/* 2026 Month Grid */}
      <Card style={{padding:20}}>
        <p style={{margin:"0 0 14px",fontWeight:700,fontSize:15,color:"#FFFFFF"}}>2026 Months</p>
        {/* Warning banner */}
        <div style={{padding:"12px 16px",background:"#1a1000",border:"1px solid #fde68a",borderRadius:10,marginBottom:16,display:"flex",gap:10}}>
          <span style={{fontSize:18,flexShrink:0}}>⚠️</span>
          <div>
            <p style={{margin:0,fontWeight:600,fontSize:13,color:"#F59E0B"}}>Important</p>
            <p style={{margin:"3px 0 0",fontSize:12,color:"#F59E0B99"}}>You can only close past months. Closing a month will freeze all resource costs, allocations, and reset employee hours to {HPM} for the next period.</p>
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
          {MC_MONTHS.map(m=>{
            ...
            return(
              <div key={m.v} style={{padding:16,border:`1px solid ${closed?"#22C55E44":"#262626"}`,borderRadius:12,background:"#161616",display:"flex",flexDirection:"column",gap:8}}>
                <div style={{display:"flex",alignItems:"center",gap:7}}>
                  <span style={{fontSize:16}}>{closed?"🔒":"📅"}</span>
                  <p style={{margin:0,fontWeight:600,fontSize:13,color:"#FFFFFF"}}>{m.l}</p>
                </div>
                {closed&&<Bdg bg="#d1fae5" color="#22C55E">Closed</Bdg>}
      ...
      <Card style={{overflow:"hidden"}}>
        <div style={{padding:"16px 20px",borderBottom:"1px solid #1E1E1E"}}>
          <p style={{margin:0,fontWeight:700,fontSize:15,color:"#FFFFFF"}}>Closed Months History</p>
        </div>
        {closedSummary.length===0?(
          <div style={{textAlign:"center",padding:"48px",color:"#777777"}}>
            <div style={{fontSize:40,marginBottom:12}}>📅</div>
              ...
              <tbody>{closedSummary.map(m=>{
                ...
                return(
                  <tr key={m.month}>
                    ...
                    <TD align="right" style={{color:"#22C55E",fontWeight:600}}>{SAR(m.totalRetainer)}</TD>
                    <TD align="right" style={{color:"#d97706"}}>{SAR(m.totalCost)}</TD>
                    <TD align="right" style={{fontWeight:700,color:m.totalProfit>=0?"#22C55E":"#EF4444"}}>{SAR(m.totalProfit)}</TD>
      ...
      <Modal open={previewModal} onClose={()=>{setPreviewModal(false);setBlockError(null);}} title={`Confirm Monthly Close — ${MC_MONTHS.find(m=>m.v===closingMonth)?.l||""}`}>
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{padding:"10px 14px",background:"#161616",borderRadius:9}}>
            <p style={{margin:0,fontSize:13,color:"#B3B3B3"}}>The following data will be frozen. Employee hours reset to {HPM} for the next period.</p>
          </div>
          <div style={{maxHeight:320,overflowY:"auto",border:"1px solid #262626",borderRadius:9}}>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr>
              ...
              <tbody>{previewData.map((item,i)=>(
                <tr key={i}>
                  <td style={{padding:"7px 13px",fontFamily:"monospace",fontSize:11,borderBottom:"1px solid #1E1E1E"}}>{item.contract_number}</td>
                  <TD>{item.client_name}</TD>
                  <TD align="right">{item.allocated_hours}h</TD>
                  <TD align="right" style={{color:"#d97706"}}>{SAR(item.resource_cost)}</TD>
                  <TD align="right" style={{color:"#22C55E"}}>{SAR(item.monthly_retainer)}</TD>
                  <TD align="right" style={{fontWeight:700,color:item.profit>=0?"#22C55E":"#EF4444"}}>{SAR(item.profit)}</TD>
                </tr>
              ))}</tbody>
            </table>
          </div>
          {previewData.length===0&&<p style={{textAlign:"center",color:"#777777",fontSize:13}}>No active contracts with allocations found for this month.</p>}
          {blockError&&(
        ...
        <div style={{maxHeight:480,overflowY:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>{snapshots.filter(s=>s.month===detailMonth).map((s,i)=>{
              ...
              return(
                <tr key={i}>
                  <td style={{padding:"8px 13px",fontFamily:"monospace",fontSize:11,borderBottom:"1px solid #1E1E1E"}}>{s.contract_number}</td>
                  ...
                  <TD align="right" style={{color:"#22C55E"}}>{SAR(s.monthly_retainer)}</TD>
                  <TD align="right" style={{fontWeight:700,color:s.profit>=0?"#22C55E":"#EF4444"}}>{SAR(s.profit)}</TD>
                  <TD align="center">{marginBadge(margin)}</TD>
                </tr>
              );
            })}</tbody>
      ...
      <Modal open={empModal} onClose={()=>setEmpModal(false)} title={`${empMonth?fmtLong(empMonth):""} — Employee Breakdown`}>
        <div style={{maxHeight:480,overflowY:"auto"}}>
          {getEmpBreakdown(empMonth||"").length===0
            ?<p style={{textAlign:"center",color:"#777777",padding:"32px 0",fontSize:13}}>No allocations found for this month</p>
            :(
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead><tr>
                ...
                <tbody>{getEmpBreakdown(empMonth||"").map((e,i)=>{
                  ...
                  const uc=util>=100?"#22C55E":util>=50?"#d97706":"#EF4444";
                  return(
                    <tr key={i}>
                      <TD><strong>{e.name}</strong></TD>
                      <TD><Bdg bg="#f1f5f9" color="#475569">{e.department}</Bdg></TD>
                      <TD style={{fontSize:12,color:"#777777"}}>{[...new Set(e.clients)].join(", ")||"—"}</TD>
...
function ExpenseCharts({expenses}){
  ...
  const EXP_COLORS=["#1DC99A","#f59e0b","#1DC99A","#ef4444","#3b82f6","#1DC99A"];
  return(
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
      <Card style={{padding:16,gridColumn:"1/-1"}}>
        <p style={{margin:"0 0 10px",fontWeight:700,fontSize:13,color:"#FFFFFF"}}>Expenses by Contract (Approved vs Draft)</p>
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={byContract} margin={{top:4,right:12,left:0,bottom:4}}>
            ...
            <Tooltip formatter={v=>fmt(v)} contentStyle={{borderRadius:8,border:"1px solid #262626",background:"#161616",color:"#FFFFFF"}}/>
            <Legend wrapperStyle={{fontSize:11,color:"#777777"}}/>
            <Bar dataKey="approved" name="Approved" fill="#1DC99A" radius={[3,3,0,0]}/>
            <Bar dataKey="draft"    name="Draft"    fill="#f59e0b" radius={[3,3,0,0]}/>
          </BarChart>
        </ResponsiveContainer>
      </Card>
      <Card style={{padding:16}}>
        <p style={{margin:"0 0 10px",fontWeight:700,fontSize:13,color:"#FFFFFF"}}>Expense Distribution by Type</p>
        <ResponsiveContainer width="100%" height={200}>
          <PieChart>
      ...
      <Card style={{padding:16}}>
        <p style={{margin:"0 0 10px",fontWeight:700,fontSize:13,color:"#FFFFFF"}}>Expenses by Department</p>
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={byDept} layout="vertical" margin={{top:4,right:12,left:8,bottom:4}}>
            ...
            <Tooltip formatter={v=>fmt(v)} contentStyle={{borderRadius:8,border:"1px solid #262626",background:"#161616",color:"#FFFFFF"}}/>
            <Bar dataKey="total" name="Total" fill="#1DC99A" radius={[0,3,3,0]}/>
          </BarChart>
        </ResponsiveContainer>
      </Card>
      {profitData.length>0&&(
        <Card style={{padding:16,gridColumn:"1/-1"}}>
          <p style={{margin:"0 0 10px",fontWeight:700,fontSize:13,color:"#FFFFFF"}}>Average Profit % by Contract</p>
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={profitData} margin={{top:4,right:12,left:0,bottom:4}}>
              ...
              <Tooltip formatter={v=>`${v}%`} contentStyle={{borderRadius:8,border:"1px solid #262626",background:"#161616",color:"#FFFFFF"}}/>
              <Bar dataKey="avg_profit" name="Avg Profit %">
                {profitData.map((e,i)=><Cell key={i} fill={e.avg_profit>=30?"#1DC99A":e.avg_profit>=10?"#f59e0b":"#ef4444"}/>)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </Card>
...
function ContractExpensesPage(){
  ...
  const profitColor=pct=>{const n=parseFloat(pct);if(isNaN(n))return"#64748b";if(n>=30)return"#22C55E";if(n>=10)return"#d97706";return"#EF4444";};
  const statusBg=s=>s==="Approved"?{bg:"#d1fae5",col:"#22C55E"}:{bg:"#fef9c3",col:"#d97706"};
  ...
  const TH=({children,align="left"})=><th style={{padding:"9px 13px",textAlign:align,fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626",whiteSpace:"nowrap"}}>{children}</th>;
  const TD=({children,align="left",style={}})=><td style={{padding:"9px 13px",textAlign:align,fontSize:12,borderBottom:"1px solid #1E1E1E",...style}}>{children}</td>;
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10}}>
        <div>
          <h1 style={{fontSize:26,fontWeight:800,color:"#FFFFFF",margin:0}}>Contract/Project Expenses</h1>
          <p style={{fontSize:13,color:"#777777",marginTop:3}}>Track and manage expenses per contract/project</p>
        </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12}}>
        ...
        <Card style={{padding:18}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:40,height:40,borderRadius:10,background:"#1E1E1E",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>💰</div>
            <div><p style={{margin:0,fontSize:11,color:"#777777"}}>Total Expenses</p><p style={{margin:"2px 0 0",fontSize:17,fontWeight:800,color:"#FFFFFF"}}>{SAR(totalExp)}</p></div>
          </div>
        </Card>
        {/* Approved */}
        <Card style={{padding:18}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:40,height:40,borderRadius:10,background:"#061510",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>✅</div>
            <div><p style={{margin:0,fontSize:11,color:"#777777"}}>Approved</p><p style={{margin:"2px 0 0",fontSize:17,fontWeight:800,color:"#22C55E"}}>{SAR(approvedExp)}</p></div>
          </div>
        </Card>
        {/* Draft */}
        <Card style={{padding:18}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:40,height:40,borderRadius:10,background:"#1a1400",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>📝</div>
            <div><p style={{margin:0,fontSize:11,color:"#777777"}}>Draft</p><p style={{margin:"2px 0 0",fontSize:17,fontWeight:800,color:"#d97706"}}>{SAR(draftExp)}</p></div>
          </div>
        </Card>
        {/* By Type */}
        <Card style={{padding:16}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
            <p style={{margin:0,fontSize:11,color:"#777777"}}>By Type</p>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:3}}>
            {byTypeCards.map(({type,total})=>(
              <div key={type} style={{display:"flex",justifyContent:"space-between",fontSize:12}}>
                <span style={{color:"#777777"}}>{type}</span>
                <span style={{fontWeight:600,color:"#FFFFFF"}}>{SAR(total)}</span>
              </div>
            ))}
          </div>
        </Card>
      ...
      <div style={{position:"relative",maxWidth:320}}>
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search expenses..."
          style={{width:"100%",padding:"8px 12px 8px 30px",border:"1px solid #262626",borderRadius:9,fontSize:13,outline:"none",background:"#161616",boxSizing:"border-box"}}/>
      </div>
      ...
      <Card style={{overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>
              {filtered.map((e,idx)=>{
                ...
                return(
                  <tr key={e.id} style={{background:idx%2===0?"#161616":"#111111"}}>
                    <td style={{padding:"9px 13px",fontFamily:"monospace",fontSize:11,fontWeight:600,color:"#B3B3B3",borderBottom:"1px solid #1E1E1E"}}>{e.expense_number||"—"}</td>
                    <TD style={{fontSize:11,color:"#777777"}}>{e.request_date||"—"}</TD>
                    <td style={{padding:"9px 13px",fontFamily:"monospace",fontSize:11,color:"#777777",borderBottom:"1px solid #1E1E1E"}}>{e.contract_number||"—"}</td>
                    ...
                    <TD>{e.vendor_name||"—"}</TD>
                    <TD style={{fontSize:11,color:"#777777"}}>{e.department?.replace(" Department","")||"—"}</TD>
          ...
          {filtered.length===0&&<div style={{textAlign:"center",padding:"48px",color:"#777777"}}><div style={{fontSize:40,marginBottom:12}}>🧾</div><p style={{fontSize:14}}>No expenses found</p></div>}
        </div>
      </Card>
      ...
      <Modal open={modalOpen} onClose={close} title={editing?"Edit Expense":"Add New Expense"}>
        <form onSubmit={handleSubmit}>
          <div style={{display:"flex",flexDirection:"column",gap:13}}>
            {/* Expense # + Date */}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <div><Lbl>Expense Number</Lbl><input value={editing?form.expense_number||"—":"Auto-generated on save"} readOnly style={{width:"100%",padding:"8px 11px",border:"1px solid #262626",borderRadius:8,fontSize:12,fontFamily:"monospace",color:"#777777",background:"#161616",boxSizing:"border-box"}}/></div>
            ...
            {form.contract_id&&(
              <div style={{padding:"12px 14px",background:"#111111",borderRadius:10,border:"1px solid #262626"}}>
                <p style={{margin:"0 0 10px",fontSize:11,fontWeight:700,color:"#777777",textTransform:"uppercase",letterSpacing:".05em"}}>Contract Details (Read Only)</p>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                  {[["Contract Number",form.contract_number],["Customer Name",form.client_name],["Category",form.contract_category],["Total Value (SAR)",Number(form.total_contract_value||0).toLocaleString()],["Start Date",form.contract_start_date],["End Date",form.contract_end_date]].map(([l,v])=>(
                    <div key={l}><p style={{margin:0,fontSize:10,color:"#777777"}}>{l}</p><p style={{margin:"1px 0 0",fontSize:13,fontWeight:600,color:"#FFFFFF"}}>{v||"—"}</p></div>
                  ))}
                  {form.contract_notes&&<div style={{gridColumn:"1/-1"}}><p style={{margin:0,fontSize:10,color:"#777777"}}>Contract Notes</p><p style={{margin:"1px 0 0",fontSize:13,color:"#B3B3B3"}}>{form.contract_notes}</p></div>}
                </div>
              </div>
            )}
            {/* Editable fields */}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              ...
              <div><Lbl>Previous Requested Total</Lbl><input value={form.previous_requested_total_amount} readOnly style={{width:"100%",padding:"8px 11px",border:"1px solid #262626",borderRadius:8,fontSize:13,color:"#777777",background:"#161616",boxSizing:"border-box"}}/><p style={{margin:"3px 0 0",fontSize:10,color:"#777777"}}>Auto-calculated from existing expenses</p></div>
            ...
            <div><Lbl>Notes *</Lbl><textarea value={form.notes} onChange={e=>updF("notes",e.target.value)} placeholder="Additional notes..." rows={2} required style={{width:"100%",padding:"8px 11px",border:"1px solid #262626",borderRadius:8,fontSize:13,color:"#FFFFFF",outline:"none",resize:"vertical",boxSizing:"border-box"}}/></div>
            {/* Profit calc */}
            {form.contract_id&&(
              <div style={{padding:"12px 16px",background:"#161616",borderRadius:10,display:"flex",alignItems:"center",gap:14}}>
                <div style={{width:38,height:38,borderRadius:9,background:"#161616",border:"1px solid #262626",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>📈</div>
                <div>
                  <p style={{margin:0,fontSize:11,color:"#777777",fontWeight:600}}>Project Profit % (Auto-calculated)</p>
                  ...
                  <p style={{margin:0,fontSize:10,color:"#777777"}}>= (Contract Value − Previous − Amount) ÷ Contract Value × 100</p>
                </div>
              </div>
            )}
            {/* Attachment placeholder */}
            <div><Lbl>Attachment</Lbl>
              <div style={{border:"2px dashed #e2e8f0",borderRadius:10,padding:"16px 20px",textAlign:"center",color:"#777777",cursor:"not-allowed"}}>
...
function SystemUsersPage(){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      {/* Header */}
      <div>
        <h1 style={{fontSize:26,fontWeight:800,color:"#FFFFFF",margin:0}}>System Users</h1>
        <p style={{fontSize:13,color:"#777777",marginTop:3}}>Manage users and role permissions</p>
      </div>

      {/* Tab bar */}
      <div style={{display:"flex",gap:4,background:"#1E1E1E",borderRadius:10,padding:4,maxWidth:340}}>
        {[["users","👥 Users"],["roles","🛡 Role Permissions"]].map(([v,l])=>(
          <button key={v} onClick={()=>setTab(v)} style={{flex:1,padding:"8px 12px",borderRadius:8,border:"none",background:tab===v?"#1E1E1E":"transparent",fontWeight:tab===v?700:500,fontSize:13,color:tab===v?"#FFFFFF":"#777777",cursor:"pointer",boxShadow:tab===v?"0 1px 3px rgba(0,0,0,.1)":"none"}}>{l}</button>
        ))}
      </div>
      ...
      {tab==="users"&&(
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          {/* Invite card */}
          <Card style={{padding:20}}>
            <p style={{margin:"0 0 14px",fontWeight:700,fontSize:14,color:"#FFFFFF",display:"flex",alignItems:"center",gap:8}}>➕ Invite New User</p>
            <form onSubmit={handleInvite} style={{display:"flex",gap:10,flexWrap:"wrap"}}>
              <input type="email" value={inviteEmail} onChange={e=>setInviteEmail(e.target.value)}
                placeholder="Enter email address" required
                style={{flex:1,minWidth:200,padding:"9px 12px",border:"1px solid #262626",borderRadius:9,fontSize:13,outline:"none",background:"#1E1E1E",color:"#FFFFFF"}}/>
          ...
          <Card style={{overflow:"hidden"}}>
            <div style={{padding:"16px 20px",borderBottom:"1px solid #1E1E1E"}}>
              <p style={{margin:0,fontWeight:700,fontSize:14,color:"#FFFFFF"}}>User Management</p>
            </div>
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead><tr>
                ...
                <tbody>{sortedUsers.map((u,idx)=>{
                  ...
                  const statusCol=u.status==="active"?"#22C55E":"#d97706";
                  const statusLabel=u.status==="active"?"Active":"Pending Invitation";
                  return(
                    <tr key={u.id} style={{background:idx%2===0?"#161616":"#111111"}}>
                      <TD>
                        <div style={{display:"flex",alignItems:"center",gap:10}}>
                          <div style={{width:34,height:34,borderRadius:9,background:"#262626",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:13,color:"#B3B3B3",flexShrink:0}}>
                      ...
                      <TD><Bdg bg={u.role==="admin"?"#0d1f1a":"#f1f5f9"} color={u.role==="admin"?"#1DC99A":"#475569"}>{u.role==="admin"?"Admin":"Manager"}</Bdg></TD>
                      <TD>
                        {u.role!=="admin"?(
                          <select value={assignedRole?.id||"none"} onChange={e=>assignRole(u.email,e.target.value,assignedRole?.id)}
                            style={{padding:"5px 8px",border:"1px solid #262626",borderRadius:7,fontSize:12,color:"#FFFFFF",background:"#161616",cursor:"pointer",width:150}}>
                            <option value="none">No role</option>
                            {roles.map(r=><option key={r.id} value={r.id}>{r.role_name}</option>)}
                          </select>
                        ):<span style={{fontSize:12,color:"#777777"}}>Full access</span>}
                      </TD>
                      <TD>
                        {u.departments?.length>0
                          ...
                          :<span style={{fontSize:12,color:"#777777"}}>None</span>}
                      </TD>
                      <TD align="center">
                        <div style={{display:"flex",justifyContent:"center",gap:4}}>
                          {!u.isPending&&u.email!==CURRENT_USER?.email&&<Btn variant="ghost" size="sm" title="Impersonate" style={{color:"#1DC99A"}} onClick={()=>alert(`Now viewing as ${u.full_name||u.email}`)}>👁</Btn>}
                          <Btn variant="ghost" size="sm" onClick={()=>openEditUser(u)} title="Edit">✏️</Btn>
                          {u.status==="invited"&&<Btn variant="ghost" size="sm" style={{color:"#1DC99A"}} onClick={()=>resendInvite(u.email)} title="Resend invite">✉️</Btn>}
          ...
          {/* Your Account card */}
          <Card style={{padding:20}}>
            <p style={{margin:"0 0 14px",fontWeight:700,fontSize:14,color:"#FFFFFF"}}>Your Account</p>
            <div style={{display:"flex",alignItems:"center",gap:16}}>
              <div style={{width:56,height:56,borderRadius:14,background:"#262626",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:20,color:"#B3B3B3",flexShrink:0}}>
                {(CURRENT_USER?.full_name||CURRENT_USER?.email||"A").charAt(0).toUpperCase()}
              </div>
              <div>
                <p style={{margin:0,fontWeight:700,fontSize:16,color:"#FFFFFF"}}>{CURRENT_USER?.full_name||"Admin"}</p>
                <p style={{margin:"2px 0 6px",fontSize:13,color:"#777777"}}>{CURRENT_USER?.email||"admin@company.com"}</p>
                <Bdg bg="#0d1f1a" color="#1DC99A">Admin</Bdg>
              </div>
            </div>
          </Card>
        </div>
      ...
      {tab==="roles"&&(
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div><p style={{margin:0,fontWeight:700,fontSize:15,color:"#FFFFFF"}}>Role Permissions</p><p style={{margin:"2px 0 0",fontSize:12,color:"#777777"}}>Define what each role can access</p></div>
            <Btn variant="primary" onClick={openCreateRole}>＋ Create Role</Btn>
          </div>

          {roles.length===0?(
            <Card style={{padding:48,textAlign:"center",color:"#777777"}}>
              <div style={{fontSize:40,marginBottom:12}}>🛡</div>
              <p style={{fontSize:14,fontWeight:600,color:"#777777"}}>No roles created yet</p>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              {roles.map(role=>(
                <Card key={role.id} style={{padding:18}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                    <div style={{display:"flex",alignItems:"center",gap:12}}>
                      <div style={{width:40,height:40,borderRadius:10,background:"#0d1f1a",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>🛡</div>
                      <div>
                        <p style={{margin:0,fontWeight:700,fontSize:15,color:"#FFFFFF"}}>{role.role_name}</p>
                        <p style={{margin:"1px 0 0",fontSize:12,color:"#777777"}}>{role.assigned_users?.length||0} users assigned</p>
                      </div>
                    </div>
                  ...
                  <div style={{overflowX:"auto"}}>
                    <table style={{width:"100%",borderCollapse:"collapse"}}>
                      <thead><tr>
                        <th style={{padding:"7px 12px",textAlign:"left",fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626",width:160}}>Module</th>
                        {["View","Create","Edit","Delete"].map(h=><th key={h} style={{padding:"7px 12px",textAlign:"center",fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626",width:80}}>{h}</th>)}
                      </tr></thead>
                      <tbody>{SU_ENTITIES.map((ent,i)=>{
                        const p=role.permissions?.[ent.key]||{};
                        return(
                          <tr key={ent.key} style={{background:i%2===0?"#161616":"#111111"}}>
                            <td style={{padding:"7px 12px",fontWeight:600,fontSize:13,borderBottom:"1px solid #1E1E1E"}}>{ent.name}</td>
                            {["view","create","edit","delete"].map(perm=>(
                              <td key={perm} style={{padding:"7px 12px",textAlign:"center",borderBottom:"1px solid #1E1E1E"}}>
                                {p[perm]?<span style={{color:"#22C55E",fontSize:16}}>✓</span>:<span style={{color:"#e2e8f0"}}>—</span>}
                              </td>
                            ))}
                          </tr>
                        );
                  ...
                  {role.allowed_departments?.length>0&&(
                    <div style={{marginTop:10,display:"flex",flexWrap:"wrap",gap:6,alignItems:"center"}}>
                      <span style={{fontSize:12,color:"#777777"}}>Dept Access:</span>
                  ...
                  {role.assigned_users?.length>0&&(
                    <div style={{marginTop:8,display:"flex",flexWrap:"wrap",gap:6,alignItems:"center"}}>
                      <span style={{fontSize:12,color:"#777777"}}>Assigned:</span>
      ...
      <Modal open={roleModal} onClose={closeRoleModal} title={editingRole?"Edit Role":"Create New Role"}>
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div>
            <Lbl>Permissions</Lbl>
            <div style={{border:"1px solid #262626",borderRadius:9,overflow:"hidden",marginTop:6}}>
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead><tr>
                  <th style={{padding:"8px 12px",textAlign:"left",fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626",width:150}}>Module</th>
                  <th style={{padding:"8px 12px",textAlign:"center",fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626",width:90}}>Full Access</th>
                  {["View","Create","Edit","Delete"].map(h=><th key={h} style={{padding:"8px 12px",textAlign:"center",fontSize:11,fontWeight:600,color:"#777777",background:"#161616",borderBottom:"1px solid #262626",width:72}}>{h}</th>)}
                </tr></thead>
                <tbody>{SU_ENTITIES.map((ent,i)=>{
                  ...
                  return(
                    <tr key={ent.key} style={{background:i%2===0?"#161616":"#111111"}}>
                      <td style={{padding:"8px 12px",borderBottom:"1px solid #1E1E1E"}}>
                        <p style={{margin:0,fontWeight:600,fontSize:12,color:"#FFFFFF"}}>{ent.name}</p>
                        <p style={{margin:0,fontSize:10,color:"#777777"}}>{ent.desc}</p>
                      </td>
                      <td style={{padding:"8px 12px",textAlign:"center",borderBottom:"1px solid #1E1E1E"}}>
                      ...
                      {["view","create","edit","delete"].map(perm=>(
                        <td key={perm} style={{padding:"8px 12px",textAlign:"center",borderBottom:"1px solid #1E1E1E"}}>
          ...
          <div>
            ...
            <p style={{margin:"2px 0 8px",fontSize:11,color:"#777777"}}>Leave all unchecked to allow all departments.</p>
            <div style={{border:"1px solid #262626",borderRadius:9,padding:"8px 12px",display:"flex",flexDirection:"column",gap:4}}>
              {SU_DEPTS.map(dept=>(
                <label key={dept} style={{display:"flex",alignItems:"center",gap:10,padding:"6px 8px",borderRadius:7,cursor:"pointer",background:"#161616"}}>
                  ...
                  <span style={{fontSize:13,color:"#FFFFFF"}}>{dept}</span>
                </label>
              ))}
            </div>
          </div>
      ...
      <Modal open={editUserModal} onClose={()=>setEditUserModal(false)} title="Edit User">
        <div style={{display:"flex",flexDirection:"column",gap:13}}>
          ...
          <div>
            <Lbl>Departments</Lbl>
            <p style={{margin:"2px 0 8px",fontSize:11,color:"#777777"}}>Select departments this user has access to</p>
            <div style={{border:"1px solid #262626",borderRadius:9,padding:"8px 12px",display:"flex",flexDirection:"column",gap:4}}>
              {SU_DEPTS.map(dept=>(
...
function ComingSoon({page}){
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:400,color:"#777777"}}>
      <div style={{fontSize:56,marginBottom:16}}>🚧</div>
      <h2 style={{fontSize:20,fontWeight:700,color:"#B3B3B3",margin:"0 0 8px"}}>{page}</h2>
...
function AccessDenied(){
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:400,gap:14}}>
      ...
      <h2 style={{margin:0,fontSize:20,fontWeight:700,color:"#FFFFFF"}}>Access Denied</h2>
      <p style={{margin:0,fontSize:13,color:"#777777"}}>You don't have permission to view this page.</p>
      <p style={{margin:0,fontSize:12,color:"#777777"}}>Contact your administrator to request access.</p>
    </div>
  );
}
...
function PlatformApp(){
  ...
  return(
    <div style={{display:"flex",height:"100vh",fontFamily:"'Inter',system-ui,sans-serif",background:"#161616",overflow:"hidden"}}>

      {/* Sidebar */}
      <aside style={{width:240,background:"#161616",borderRight:"1px solid #262626",display:"flex",flexDirection:"column",flexShrink:0,overflowY:"auto"}}>
        {/* Logo */}
        <div style={{padding:"18px 20px",borderBottom:"1px solid #1E1E1E"}}>
          <p style={{margin:0,fontWeight:800,fontSize:15,color:"#FFFFFF"}}>Team Allocation</p>
          <p style={{margin:"2px 0 0",fontSize:11,color:"#777777"}}>Acquaint Communications</p>
        </div>
        {/* Nav */}
          ...
          {NAV.filter(item=>can(PAGE_PERM_KEY[item.id],"view")).map(item=>{
            ...
            return(
              <button key={item.id} onClick={()=>setActivePage(item.id)}
                style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",borderRadius:10,border:"none",background:active?"#1DC99A22":"transparent",color:active?"#1DC99A":"#777777",cursor:"pointer",fontSize:13,fontWeight:active?600:500,textAlign:"left",transition:"background .15s",width:"100%"}}>
        ...
        <div style={{padding:"12px 14px",borderTop:"1px solid #1E1E1E"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
            ...
            <div style={{flex:1,overflow:"hidden"}}>
              <p style={{margin:0,fontWeight:600,fontSize:12,color:"#FFFFFF",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{profile?.full_name||"User"}</p>
              <p style={{margin:0,fontSize:11,color:"#777777"}}>{profile?.role==="admin"?"Admin":"Manager"}</p>
            </div>
          </div>
          <button onClick={signOut} style={{width:"100%",padding:"7px",borderRadius:8,border:"1px solid #262626",background:"#161616",color:"#777777",fontSize:12,cursor:"pointer",fontWeight:500}}>Sign Out</button>
        </div>
      </aside>
